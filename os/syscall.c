...
int sys_mutex_create(int blocking)
{
	...
	curr_proc()->availible1[mutex_id]++;
	return mutex_id;
}
int lock_detector(const int available[LOCK_POOL_SIZE],
		    const int allocation[NTHREAD][LOCK_POOL_SIZE],
		    const int request[NTHREAD][LOCK_POOL_SIZE])
{
	int finish[NTHREAD] = { 0 };
	int work_resources[LOCK_POOL_SIZE];
	memcpy(work_resources, available, sizeof(work_resources));

	int modification_flag = 0;
	int thread_index, resource_index;

	while (1) {
		modification_flag = 0;
		thread_index = 0;
		while (thread_index < NTHREAD) {
			if (!finish[thread_index]) {
				int can_allocate = 1;
				resource_index = 0;
				while (resource_index < LOCK_POOL_SIZE) {
					if (request[thread_index][resource_index] > work_resources[resource_index]) {
						can_allocate = 0;
						break;
					}
					resource_index++;
				...
				if (can_allocate) {
					finish[thread_index] = 1;
					resource_index = 0;
					while (resource_index < LOCK_POOL_SIZE) {
						work_resources[resource_index] += allocation[thread_index][resource_index];
						resource_index++;
					}
					modification_flag = 1;
				}
...
int sys_mutex_lock(int mutex_id)
{
	...
	p->request1[tid][mutex_id]++;
	if (p->deadlock_detect_enabled )
	   {if( lock_detector(p->availible1, p->allocation1,
			    p->request1) != 0)
		return -57005;
	}
	mutex_lock(&curr_proc()->mutex_pool[mutex_id]);

	p->allocation1[tid][mutex_id]++;
	p->request1[tid][mutex_id]--;
	p->availible1[mutex_id]--;
	return 0;
}
...
int sys_semaphore_down(int semaphore_id)
{
	...
	if (p->deadlock_detect_enabled 
			...
			if(lock_detector(p->sem_available, p->sem_allocation,
			    p->sem_request) != 0)
		return -57005;
	}
